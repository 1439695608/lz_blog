    ...
    <div
      ...
      >
        <div class="spinner-holder">
          <div
            v-if="state != 2"
            :fill-color="refreshLayerColor">
            <Arrow class="arrow"/>
            <span
              v-if="state == 0"
              class="text">下拉开始刷新数据</span>
            <span
              v-if="state == 1"
              class="text">松开立即刷新</span>
          </div>
          <div
            v-if="state == 2"
            :fill-color="refreshLayerColor">
...
export default {
  ...
  computed: {
    ...
    showInfiniteLayer () {
      let contentHeight = 0
      this.content
        ? contentHeight = this.contentOffset
        : void 666
      // console.log('showInfiniteLayer: [contentHeight: '+contentHeight+', offsetHeight: '+ (this.content ? this.content.offsetHeight : void 666) +']')
  ...
  mounted () {
    ...
    this.contentOffset = this.content.offsetHeight
    ...
    this.scroller = new Scroller(render, {
      ...
      scrollOverBottom: this.scrollOverBottom
    })
    ...
    // console.log('mounted: [content_width: ' + content_width + ', content_height: ' + content_height + ']')

    this.resizeTimer = setInterval(() => {
      let {width, height} = contentSize()
      if ((width !== content_width || height !== content_height) && document.getElementById(contentId)) {
        // console.log('resize interval [width: '+width+', height: '+height+']')
        // console.log('resize interval [content_width: '+content_width+', content_height: '+content_height+']')
        content_width = width
  ...
  methods: {
    resize () {
      ...
      if (this.recoverPosition && sessionStorage.lastScrollerPosition) {
        let position = JSON.parse(sessionStorage.lastScrollerPosition)
        this.scroller.__scrollTop = position.top
        sessionStorage.removeItem('lastScrollerPosition')
      }
      this.contentOffset = content.offsetHeight
    ...
    savePosition () {
      sessionStorage.lastScrollerPosition = JSON.stringify(this.getPosition())
    },
